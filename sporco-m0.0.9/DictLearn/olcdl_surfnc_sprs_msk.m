function [D, optinf] = olcdl_surfnc_sprs_msk(D0, S, lambda, W, opt)

% olcdl_surfnc_sprs_msk -- Masked Online Convolutional Dictionary Learning
...
%
% Usage:
%       [D, optinf] = olcdl_surfnc_sprs_msk(D0, S, lambda, W, opt);
%
% Input:
...
while k <= opt.MaxMainIter
  ...
  for sp_index_x = 1:(Nx)
    for sp_index_y = 1:(Ny)
        ...
        SampleW = W(randleft:randright,randtop:randbottom, index);
        opt_sc.W = SampleW;
