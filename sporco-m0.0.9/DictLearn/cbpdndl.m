...
Pnrm = @(x) bsxfun(@rdivide, x, sqrt(sum(sum(conj(x).*x, 1), 2)));

% Projection of filter to full image size and its transpose
% (zero-pad and crop respectively)
Pzp = @(x) zpad(x, xsz(1:2));% P in (34)
PzpT = @(x) bcrop(x, dsz);% P_T in (34)
...
if opt.display
    iterate_fig = figure();
    filter_fig = figure();
    display_func(iterate_fig, filter_fig, Gf, fft2(Y), S, D, 0);
end
...
%   Y = shrink(Xr + U, (lambda/rho)*opt.L1Weight);
    Y = Shrinkcomp(Xr + U, (lambda/rho)*opt.L1Weight);
...
%   D = ifft2(Df, 'symmetric');
  ...
  %Display it.
  if opt.display
    display_func(iterate_fig, filter_fig, Gf, Yf, S, PzpT(G), k);
  end
...
function [] = display_func(iterate_fig, filter_fig, Gf, Yf, S, D, iter)
